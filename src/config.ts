...
export const defaultConfig: ClaudeSurfConfig = {
...
export function loadConfig(projectDir?: string): ClaudeSurfConfig {
...
export function validateConfig(config: ClaudeSurfConfig): string[] {
  const errors: string[] = [];
  ...
  if (config.zones.hot >= config.zones.warm) errors.push('zones.hot must be less than zones.warm');
  if (config.zones.warm >= config.zones.cold) errors.push('zones.warm must be less than zones.cold');
