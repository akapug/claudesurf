...
export function uninstallHooks(options: InstallOptions = {}): void {
  const config = loadConfig();
  const targetDir = options.targetDir || join(process.cwd(), config.hooksDir);
...
function generatePreCompactHook(config: typeof import('./config').defaultConfig, useGlue: boolean): string {
  return `#!/bin/bash
...
function generateSessionRestoreHook(config: typeof import('./config').defaultConfig, useGlue: boolean): string {
  return `#!/bin/bash
...
function generateRollingCompactionHook(config: typeof import('./config').defaultConfig, useGlue: boolean): string {
  return `#!/bin/bash
...
function generateSessionCheckpointHook(config: typeof import('./config').defaultConfig, useGlue: boolean): string {
  return `#!/bin/bash
